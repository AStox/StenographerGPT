...
lib = ffi.dlopen(None)  # Load the default system library
...
class _CoreAudio:
    ...
    def get_property(target, selector, data_type):
        ...
        err = _cac.AudioObjectGetPropertyData(target, address, 0, ffi.NULL, size, data)
    ...
    def CFString_to_str(cfstrptr):
        str_length = _cac.CFStringGetLength(cfstrptr[0])
        max_size = _cac.CFStringGetMaximumSizeForEncoding(str_length, kCFStringEncodingUTF8)
        str_buffer = ffi.new('char[]', max_size)

        err = _cac.CFStringGetCString(cfstrptr[0], str_buffer, max_size, kCFStringEncodingUTF8)
    ...
    def create_multi_output_device(device_uids):
        ...
        plugin_bundle_ref = _CoreAudio.get_property(_cac.kAudioObjectSystemObject, _cac.kAudioHardwarePropertyPlugInForBundleID, "CFStringRef")
    ...
    def get_plugins():
        ...
        kAudioHardwarePropertyPlugInBundleID = int.from_bytes(b'piid', byteorder='big')
        ...
        _cac.AudioObjectGetPropertyDataSize(
        ...
        print("ok")
        _cac.AudioObjectGetPropertyData(
            _cac.kAudioObjectSystemObject,
            address,
            0,
            ffi.NULL,
        ...
        # Iterate through plugins and retrieve bundle identifier of first plugin
        ...
        address = ffi.new("AudioObjectPropertyAddress *", [kAudioHardwarePropertyPlugInBundleID, _cac.kAudioObjectPropertyScopeGlobal, _cac.kAudioObjectPropertyElementMaster])

        for i in range(num_plugins[0]):
            plugin_id = plugins[i]
            bundle_id = ffi.new("char[256]")
            ...
            print("Plugin Bundle Identifier:", ffi.string(bundle_id).decode())
            break
...
def main():
    ...
    for device_id in device_ids:
        ...
        uid_str = _CoreAudio.CFString_to_str(uid)
